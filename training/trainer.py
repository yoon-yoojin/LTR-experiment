"""Training utilities for LTR models."""
import torch
...
class PairwiseTrainer:
    """Trainer for pairwise LTR models."""
    ...
    def __init__(
        ...
        # Setup logging
        self.logger = setup_logger('PairwiseTrainer', config.get('logging', 'log_dir'))
    ...
    def train_epoch(self) -> float:
        ...
        for batch in tqdm(self.train_loader, desc='Training'):
            doc_i, doc_j, target = batch
            doc_i = doc_i.to(self.device)
            doc_j = doc_j.to(self.device)
            target = target.to(self.device)

            # Forward pass
            loss = self.model.compute_loss(doc_i, doc_j, target)
